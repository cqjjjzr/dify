...
class TestEndUserServiceFactory:
    ...
    @staticmethod
    def create_app_and_account(transactional_database_session: Session):
        tenant = Tenant(name=f"Tenant {uuid4()}")
        transactional_database_session.add(tenant)
        ...
        transactional_database_session.add(account)
        transactional_database_session.flush()
        ...
        transactional_database_session.add(tenant_join)
        transactional_database_session.flush()
        ...
        transactional_database_session.add(app)
    ...
    def create_end_user(
        ...
        *,
        ...
        transactional_database_session.add(end_user)
        transactional_database_session.commit()
        return end_user
...
class TestEndUserServiceGetOrCreateEndUser:
    ...
    def test_get_or_create_end_user_with_custom_user_id(
        self, transactional_database_session: Session, factory: TestEndUserServiceFactory
    ):
        """Test getting or creating end user with custom user_id."""
        # Arrange
        app = factory.create_app_and_account(transactional_database_session)
    ...
    def test_get_existing_end_user(self, transactional_database_session: Session, factory: TestEndUserServiceFactory):
...
class TestEndUserServiceGetOrCreateEndUserByType:
    ...
    def test_upgrade_legacy_end_user_type(
        ...
        transactional_database_session: Session,
    ...
    def test_get_existing_end_user_matching_type(
        self, transactional_database_session: Session, factory: TestEndUserServiceFactory, caplog
    ):
    ...
    def test_create_end_user_with_different_invoke_types(
        self, transactional_database_session: Session, invoke_type: EndUserType, factory: TestEndUserServiceFactory
    ):
...
class TestEndUserServiceCreateBatch:
    ...
    def _create_multiple_apps(
        self, transactional_database_session: Session, factory: TestEndUserServiceFactory, count: int = 3
    ):
        """Create multiple apps under the same tenant."""
        first_app = factory.create_app_and_account(transactional_database_session)
        ...
        for _ in range(count - 1):
            ...
            transactional_database_session.add(app)
        transactional_database_session.commit()
        all_apps = transactional_database_session.query(App).filter(App.tenant_id == tenant_id).all()
        return tenant_id, all_apps

    def test_create_batch_empty_app_ids(self, transactional_database_session: Session):
    ...
    def test_create_batch_creates_users_for_all_apps(
        ...
        tenant_id, apps = self._create_multiple_apps(transactional_database_session, factory, count=3)
    ...
    def test_create_batch_default_session_id(
        ...
        tenant_id, apps = self._create_multiple_apps(transactional_database_session, factory, count=2)
        app_ids = [a.id for a in apps]
    ...
    def test_create_batch_all_invoke_types(
        ...
        tenant_id, apps = self._create_multiple_apps(transactional_database_session, factory, count=1)
        user_id = f"user-{uuid4()}"
